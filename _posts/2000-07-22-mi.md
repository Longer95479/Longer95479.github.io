...
ceres 自动求导的原理，参考 ceres 的官方文档。
将实数 a 附加上 n 个无穷小量单位 e_i，之后重载基本函数在 Jet 上的加减乘除，
无穷小的部分则存储了导数，在 evaluate 实数的同时也 eveluate 对偶部分（即导数），
有点像正向传播同时求导，但不考虑中间变量的导数，只考虑给定的 parameters 的导数。
...
定坐标系：global map frame, local map frame, submap fram
动坐标系：tracking frame
