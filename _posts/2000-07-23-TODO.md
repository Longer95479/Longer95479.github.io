...
NX 板左侧 micro-usb 口不能用于相机，会读不到图像
...
- [ ] (b)部分图偏大，改正。如图 3-1。
- [ ] (c)部分图表需要按照规范进行修改，图表中的文字要尽量在五号字大小，并且汉字用宋体，字母等用Times New Roman字体。
- [ ] (c)多个子图要有标注，例如图3-4等。
- [ ] (ac)技术路线不合适 -> (m)技术路线图修改图中文字描述，使其更学术，与节标题对应
- [ ] (m)图 4-1 图中文字和标题对应，4.4 部分合并在一起

写作

- [ ] (a)四级标题精简掉一部分，保留必要的部分。保留的理由：跨页，使用四级标题易于阅读
- [ ] (c)论文中的某些公式变量或运算并未给出说明，例如公式(2-16)中的?，部分公式不规范，例如矩阵的转置在p14和p15在论文中采用了不同的符号
- [ ] (c)论文存在书写错误，例如p12“该里程计主要应用于配备相机的表面覆盖无人机。
- [ ] (c)请注意第1章研究内容的描述与摘要和结论的不同写法，凝练摘要和结论，摘要中介绍研究内容和思路，结论中突出论文的创新点和取得的成果。

实验

- [ ] (a)建议增加嵌入式无人机平台硬件在环或实物验证 -> 或者弱化为使用实际录制数据集
- [ ] (a)除了定位精度，建议引入更明确、可量化的评价指标
- [ ] (c)各个表格中给出的定位误差等描述要注意合理有效位数的选择，例如表3-5

- [ ] (b)第四章 25 26 年有更新更优的对比方法吗
- [ ] (b)对比场景选择教堂和歌剧院，是否能覆盖研究所需的所有场景，论文阐述的更多是隧道、管廊等存在稀疏纹理、剧烈光照变化以及结构特征复杂场景，目前选用的场景是否覆盖描述问题，是否有局限性，如何证明其泛化能力。
  - [ ] (m)第三章加室外数据集，第四章加室内数据集
- [ ] (b)该方法应用在无人机场景下，是否需要考虑算力开销，计算复杂度等，如何保证算法在算力受限的无人机上依然保持算法的有效性和准确性。
