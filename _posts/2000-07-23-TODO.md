...
- [ ] 视差判断与 imu 判断函数可放在 processMeasurement() 函数内，会比放在 回调函数 inputFeature 里更合适
...
- [ ] zupt 检测结果有两个去路，一个是话题发布，另一个是类内状态量，用于通知后端构造残差
  - [x] 类内结果队列以适应多线程场景，存储 (时间、结果) 对；加 mutex
  - [ ] 结果需额外补充 am RGI wm，用以残差构建

- [ ] 新增用于存储 zupt 残差所需的一些信息的滑窗数组
  - [ ] 参照 imu 因子的构建思路，高频数据存入队列，弹出图像（processMeasurement）时往滑窗数组中最新的那一个存入新数据；因为该位置在上一轮的 SlideWindows 时就已经空出来了
