...
- [ ] 录制数据集
    - [ ] 标定数据集：外参标定报错，参考 [Optimization failed in kalibr_calibration_imu_camera](https://github.com/ethz-asl/kalibr/issues/41), [D435i运行VINS-mono以及Kalib标定](https://blog.csdn.net/panda_like_me/article/details/107823553)，解决办法是调大 timeOffsetPadding 0.01 -> 0.1
    - [x] 测试数据集
...
- [x] 测试
    - [x] 原 imu, 有 zupt 0.6m, 无 zupt 0.8m
    - [x] 新 imu (long_ext_cali_off)，无 zupt 0.67m
    - [x] 新 imu (long_ext_cali_off_and_OL)，有 zupt 0.34m，无 zupt 0.35m
    - [x] 新 imu (short_ext_cali_off)，无 zupt 1.2m
    - [x] 新 imu (short_ext_cali_off_and_OL)，有 zupt 0.34m，无 zupt 0.35m
    - [x] 新 imu (OL_iter)，有 zupt 0.36 m，无 zupt 0.31m；且外参迭代值（主要看 tbc）接近于 long_ext_cali_off 的结果
        - 多次在线迭代并不会让漂移一直变小，观察 tbc 发现多次迭代后偏离了真实值，需要加额外约束（如双目对齐约束）
    - [x] 新 imu 下的 vins 估计的零偏都小于 0.05 m/s^2，变化不大，应该算正确的
